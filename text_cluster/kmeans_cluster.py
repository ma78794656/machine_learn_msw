...
class TextCluster(object):
    ...
    def load_stop_words(self):
        if self.stop_words:
            with open(self.stop_words_file, encoding='utf-8') as f:
                words = f.readlines()
            self.stop_words = [w.strip() for w in words]
    ...
    def load_processfile(process_file):
        corpus_list = []
        try:
            fp = codecs.open(process_file, "r", 'utf-8')
            for line in fp:
                conline = line.strip()
...
if __name__ == "__main__":
    # 获取TextProcess对象
    tc = TextCluster()
    tc.process("./data_little.txt", "tf_Result.txt", "tfidf_Result.txt", 10, "cluster_Result.txt")
